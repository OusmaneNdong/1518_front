...
getAllDemandeur():Observable<Demandeur[]>{
  return this.httpClient.get<Demandeur[]>(this.url + "/api/demandeur/getDemandeur");
}
...
uploadImage(data: any,id: number):Observable<number>{
  const imageFormData = new FormData();
  imageFormData.append('file', data);
  return this.httpClient.post<number>(this.url + "/user/" + id, imageFormData);
}
